...
app.post('/process-video', async (req, res) => {
  try {
    ...
    if (captions) {
      // Convert SRT to ASS format (like local environment)
      const assContent = convertSRTToASS(captions);
      fs.writeFileSync(subsPath, assContent);

      console.log('ASS file created:', subsPath);
      console.log('ASS content preview:', assContent.substring(0, 200));
      console.log('Full ASS content:', assContent);

      // Use subtitles filter with proper escaping (like local environment)
      const subsPathEscaped = subsPath.replace(/\\/g, '/').replace(/:/g, '\\:');
      finalFilter = `${baseFilter},subtitles='${subsPathEscaped}':force_style='FontSize=36,Outline=2'`;
...
function parseSRTToDrawtext(srtContent) {
  ...
  while (currentIndex < lines.length) {
    ...
    if (line.includes('-->')) {
      ...
      if (textLines.length > 0) {
        ...
        const startTime = convertSRTTimeToSeconds(start);
        const endTime = convertSRTTimeToSeconds(end);

        drawtextFilters.push(
          `drawtext=text='${text}':fontsize=24:fontcolor=white:box=1:boxcolor=black@0.5:boxborderw=5:x=(w-text_w)/2:y=h-th-20:enable='between(t,${startTime},${endTime})'`
        );
      }
    } else {
      currentIndex++;
...
function convertSRTToASS(srtContent) {
  console.log('SRT content to parse:', srtContent);

  // Handle JSON response format
  let actualSrtContent = srtContent;
  if (typeof srtContent === 'string' && srtContent.startsWith('{')) {
    try {
      const parsed = JSON.parse(srtContent);
      if (parsed.success && parsed.captions) {
        actualSrtContent = parsed.captions;
        console.log('Extracted SRT from JSON:', actualSrtContent);
      }
    } catch (e) {
  ...
  const lines = actualSrtContent.split('\n');
  console.log('SRT lines count:', lines.length);
  const assLines = [
    '[Script Info]',
  ...
  while (currentIndex < lines.length) {
    const line = lines[currentIndex].trim();
    console.log(`Processing line ${currentIndex}: "${line}"`);

    if (line.match(/^\d+$/)) {
      // This is a subtitle number, skip it
      console.log('Found subtitle number:', line);
      currentIndex++;
      continue;
    }
    ...
    if (line.includes('-->')) {
      ...
      if (textLines.length > 0) {
        ...
        console.log(`Creating dialogue: ${startTime} -> ${endTime}: "${text}"`);
  ...
  console.log(`Total dialogues created: ${dialogueCount}`);
